...
namespace RedStream
{
    ...
    public abstract class ParticleSystem : GameComponent
    {
        ...
        protected void LoadContent()
        {
            ...
            vertexBuffer = new DynamicVertexBuffer(RedStream.Game.GraphicsDevice, ParticleVertex.VertexDeclaration,
            ...
            indexBuffer = new IndexBuffer(RedStream.Game.GraphicsDevice, typeof(ushort), indices.Length, BufferUsage.WriteOnly);
        ...
        {
            Effect effect = RedStream.Content.GetEffect("ParticleEffect");
            ...
            Texture2D texture = RedStream.Content.GetTexture(settings.TextureName);
        ...
        {
            GraphicsDevice device = RedStream.Game.GraphicsDevice;
            ...
            if (firstActiveParticle != firstFreeParticle)
            {
                ...
                SetCamera(RedStream.Scene.Camera.View, RedStream.Scene.Camera.Proj);
        ...
        /// </summary>
        public void AddParticle(Vector3 position, Vector3 velocity, float scale)
        {
            ...
            velocity.X += horizontalVelocity * (float)Math.Cos(horizontalAngle) * scale * scale;
            velocity.Z += horizontalVelocity * (float)Math.Sin(horizontalAngle) * scale * scale;

            // Add in some random amount of vertical velocity.
            velocity.Y += MathHelper.Lerp(settings.MinVerticalVelocity,
                                          settings.MaxVerticalVelocity,
                                          (float)random.NextDouble()) * scale * scale;
            ...
            /*Color randomValues = new Color((byte)random.Next(255),
                                           ...
                                           (byte)random.Next(255),
                                           (byte)random.Next(255));*/
            Color randomValues = new Color(new Vector4((float)random.NextDouble(),
                                                     (float)((random.NextDouble() * 0.5 + 0.5) * scale),
                                                     (float)random.NextDouble(),
                                                     (float)random.NextDouble()));
        ...
        public void AddParticles(int numParticles, Vector3 position, float scale, Vector3 velocity)
        {
            for (int i = 0; i < numParticles; i++)
                AddParticle(position, velocity, scale);
        }
